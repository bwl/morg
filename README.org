...
** Org Mode Power
- *denote*: Simple, file-based Zettelkasten (no database required)
...
** Modern Emacs (2024+ Stack)
- *Vertico + Consult + Orderless*: Modern completion (replaces Ivy/Helm)
- *Corfu + Cape*: Fast, minimal code completion (replaces Company)
- *Embark*: Contextual actions on any target
- *project.el*: Built-in project management (replaces Projectile)
- *Treemacs*: Modern file tree with workspaces (replaces NeoTree)
- Requires Emacs 29+ with native-comp, tree-sitter, and eglot
...
** Developer Tools
- Multiple cursors, Git (Magit), file tree (Treemacs)
...
Morg uses the built-in =project.el= (Emacs 29+) for project management. It's lightweight and integrates with the modern completion stack.

| Binding          | Description                 |
|------------------+-----------------------------|
| =Ctrl-Cmd-p=     | Switch project              |
| =Cmd-p=          | Find file in project        |
| =Cmd-Shift-f=    | Search in project (ripgrep) |
...
Projects are auto-detected via =.git=, =.project=, or other VCS markers.

** Git

*** Git gutter

Changes to the current file are shown in the gutter on left side. You can change the appearance of those symbols: search for =git-gutter= section in =init.el=. See [[https://github.com/syohex/emacs-git-gutter][Git-gutter docs]] for more info.
...
| =Cmd-Shift-b=   | Toggle file tree (Treemacs)                  |
...
*** Denote: Knowledge Management

[[https://protesilaos.com/emacs/denote][Denote]] provides simple, file-based note-taking (Zettelkasten). No database required—just plain Org files with smart naming. Notes are stored in =~/org/notes=.
...
| =C-c n L= | Find and open a backlink            |
| =C-c n g= | Grep through notes                  |
| =C-c n j= | Create today's journal entry        |
| =C-c n r= | Rename file based on front matter   |

Notes are named with the pattern: =DATE--TITLE__KEYWORDS.org=

Built-in keywords: =reference=, =project=, =idea=, =meeting=, =journal=
...
#+BEGIN_SRC sh
mkdir -p ~/org/notes/journal ~/org/notes/references ~/org/notes/projects ~/org/images
